...
var self_details = { 'topics': ['pork'], 'port': 8082, 'host_name': 'broker-service-2' }
var neighbour_node_details = [{ 'port': 8080, 'host_name': 'broker-service' }, {'port':8081,'host_name':'broker-service-1'}]
...
app.post("/advertise", (req, res) => {
    ...
    subInfoData.find({ subscriberName: req.body.subscriberName }, (err, docs) => {
        if (docs.length) {
            ...
            docs[0].save((err) => {
                //console.log("isAdvertise set to True");
                if (err) {
                    console.log(err);
                }
            });
